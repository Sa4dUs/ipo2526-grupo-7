...
function switchScreen(screenId) {
	...
	document.querySelectorAll('.nav-tab').forEach((t) => {
		t.classList.toggle(
			'active',
			t.getAttribute('onclick') && t.getAttribute('onclick').includes(screenId),
		);
	});
...
function openActionDetails(cat) {
	...
	document
		.querySelectorAll('.action-field-set')
		.forEach((fs) => (fs.style.display = 'none'));
	...
	if (cat === 'Transporte') {
		...
		desc.innerText =
			'Calcula el ahorro por elegir opciones vegetales o locales.';
...
function toggleContrast() {
	const body = document.body;
	const newVal =
		body.getAttribute('data-contrast') === 'high' ? 'standard' : 'high';
...
function toggleMotion() {
	const body = document.body;
	const newVal =
		body.getAttribute('data-motion') === 'reduce' ? 'standard' : 'reduce';
...
function speak(text) {
	if (
		document.body.getAttribute('data-narrator') === 'on' &&
...
function selectCalcOption(btn, group) {
	document
		.querySelectorAll(`.calc-option[data-group="${group}"]`)
...
function calcNextStep(step) {
	for (let i = 1; i <= CALC_STEPS; i++) {
		...
		if (dot)
			dot.style.background = i <= step ? 'var(--primary)' : 'var(--line)';
	}
...
function computeAndShowResult() {
	...
	const total =
		transportCO2 + dietCO2 + heatingCO2 + flightsCO2 + consumptionCO2;
